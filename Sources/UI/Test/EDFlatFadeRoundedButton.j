...
@implementation EDFlatFadeRoundedButton : CPBox
{
    CPTextField        _titleTF;
    CPBox              _mouseoverBox;
    CPBox              _hightlightBox;
...
- (id)initWithFrame:(CGRect)iFrame fadeDuration:(double)iDuration cornerRadius:(double)iCornerRadius
{
    if (self = [super initWithFrame:iFrame])
    {
        _duration       = iDuration;
        _cornerRadius   = iCornerRadius;
        [self _init:iFrame];
    }
...
//utilisation de box pour faire des boutons aux coins arrondis
//attention: si on garde aussi le bord, ça fait pas joli, faut mettre les bords transparents
- (void)_init:(CGRect)iFrame
{
 ...
 //  [self setFillColor:[CPColor greenColor]];

   var fullFrame = CGRectMake(0, 0, iFrame.size.width, iFrame.size.height);


    //Over color
    _mouseoverBox = [[CPBox alloc] initWithFrame:fullFrame];
   // [_mouseoverView setBackgroundColor:[CPColor greenColor]];
    [self addSubview:_mouseoverBox];
    [_mouseoverBox setAlphaValue:0];

    _hightlightBox = [[CPBox alloc] initWithFrame:fullFrame];
    [self addSubview:_hightlightBox];
    [_hightlightBox setHidden:YES];

    //Corner radius
    //[self setValue:_cornerRadius forThemeAttribute:@"corner-radius"];
    //[_mouseoverBox setValue:_cornerRadius forThemeAttribute:@"corner-radius"];
    //[_hightlightBox setValue:_cornerRadius forThemeAttribute:@"corner-radius"];
    [self setCornerRadius:_cornerRadius];
    [_mouseoverBox setCornerRadius:_cornerRadius];
    [_hightlightBox setCornerRadius:_cornerRadius];

    [self setBorderWidth:0.0];
    [_mouseoverBox setBorderWidth:0.0];
    [_hightlightBox setBorderWidth:0.0];

    [self setBorderColor:[CPColor clearColor]];
    [_mouseoverBox setBorderColor:[CPColor clearColor]];
    [_hightlightBox setBorderColor:[CPColor clearColor]];

    //[self setBorderType:CPLineBorder];
    //[_mouseoverBox setBorderType:CPLineBorder];
    //[_hightlightBox setBorderType:CPLineBorder];

    //Title
//    _title  = [[CPTextField alloc] initWithFrame:CGRectMake(44, 5, aFrame.size.width - 44, 20)];
    _titleTF  = [[CPTextField alloc] initWithFrame:fullFrame];

    [_titleTF setCenter:CGPointMake(iFrame.size.width / 2.0 , iFrame.size.height / 2.0)];
//    [_title setCenter:CGPointMake(centerX , centerY)];

    [_titleTF setValue:CPCenterTextAlignment forThemeAttribute:@"vertical-alignment"];
    [_titleTF setValue:CPCenterTextAlignment forThemeAttribute:@"alignment"];

    var inset = CGInsetMake(10, 10, 10, 10);
    [_titleTF setValue:inset forThemeAttribute:@"content-inset"];

 //   [_title setTextColor:[CPColor whiteColor]];
 //   [_title setTextColor:[self valueForThemeAttribute:@"text-color"]];
    [_titleTF setTextColor:[CPColor whiteColor]];
    [_titleTF setAutoresizingMask:CPViewWidthSizable | CPViewHeightSizable];
    [_titleTF setLineBreakMode:CPLineBreakByWordWrapping];
    //[_titleTF setStringValue:@"TEST"];
    [_titleTF setEditable:NO];

    [self addSubview:_titleTF];


    _fadeInAnimation    = [self _fadeAnimation:_mouseoverBox start:0.0 end:1.0 duration:_duration];
    _fadeOutAnimation   = [self _fadeAnimation:_mouseoverBox start:1.0 end:0.0 duration:_duration];
...
{
    [_titleTF setStringValue:iTitle];
}
...
{
    [_titleTF setTextColor:aColor];
}

- (void)setMouseHoverColor:(CPColor)aColor
{
    [_mouseoverBox setFillColor:aColor];
}

- (void)setHighlightColor:(CPColor)aColor
{
    [_hightlightBox setFillColor:aColor];
}
